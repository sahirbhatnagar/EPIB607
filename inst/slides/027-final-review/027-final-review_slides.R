...
## ----echo=FALSE, eval=TRUE----------------------------------------------------
source("https://raw.githubusercontent.com/sahirbhatnagar/epib607/master/inst/slides/bin/plot_null_alt.R")
n <- 5
s <- 0.0080
mu0 <- -0.540
mha <- 0.99*-0.540
...
## ----echo=FALSE, eval=TRUE, fig.asp=0.601-------------------------------------
n <- 13.6
s <- 0.0080
mu0 <- -0.540
mha <- 0.99*-0.540
...
## ----fig.asp=0.601, echo = c(-1,-2,-5,-6,-7)----------------------------------
load(here::here("inst/assignments/a8/icu.rda"))
icu <- icu %>% mutate(sta.int = as.numeric(sta) - 1)
fit1 <- glm(sta ~ type, family=binomial(link=logit), data=icu)
fit2 <- glm(sta ~ type + age, family=binomial(link=logit), data=icu)
par(mfrow = c(1,2))
