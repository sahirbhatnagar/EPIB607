...
<https://github.com/sahirbhatnagar/EPIB607/blob/master/inst/data/heights_sample.csv>.) We read the
content of the file into a data frame by the name “`heights_sample`" and apply the
function “`summary`" to the data frame:

```{r, echo=TRUE}
heights_sample <- read.csv(here::here("inst/data/heights_sample.csv"))
...
be obtained from
(<https://github.com/sahirbhatnagar/EPIB607/blob/master/inst/data/heights_sample.csv>). It is a
...
```{r, echo=TRUE}
heights_population <- read.csv(file=here::here("inst/data/heights_population.csv"))
...
plot(table(heights_population$height), xlab="height",ylab="Frequency")
```
...
The object `heights_population$height` is a sequence with 100,000 entries. Think of
it as a population. We will apply the function “`sample`" to this
sequence:

```{r}
sample(heights_population$height,1)
```
...
```{r}
heights_population <- read.csv(file=here::here("inst/data/pop1.csv"))
X <- heights_population$height
```
...
[^4_1]: Such a bar plot can be produced with the expression
    “`plot(table(heights_population$height))`".
