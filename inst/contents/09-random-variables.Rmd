...
In the previous chapter we needed to define a probability for each outcome (or event in the sample space). Often times, this isn't feasible because there are too many probabilities to define and not enough data to reliably estimate each one. An alternative would be to rely on a probability model which depends on a relatively small number of statistical parameters that define the entire probability distribution. In other words, we can calculate the entire probability distribution using a formula and parameter values. 
...
random variable $Y$ has such a distribution is marked by the expression:
...
Consequently, if $Y$ is the total number of “Heads" then
$Y \sim \mathrm{Binomial}(10,0.5)$, where $n=10$ is the number of trials
and $p=0.5$ is the probability of success in each trial.

It may happen that all 10 coins turn up “Tail". In this case $Y$ is
equal to 0. It may also be the case that one of the coins turns up
“Head" and the others turn up “Tail". The random variable $Y$ will
...
sample space of $Y$ is the set of integers $\{0, 1, 2, \ldots, 10\}$.
...
of the event $\{\mbox{Y is odd}\}$ we should sum up these probabilities,
...
```{r}
pbinom(y,10,0.5)
cumsum(dbinom(y,10,0.5))
```

The output of the function “`pbinom`" is the cumulative probability
$\Prob(Y \leq y)$ that the random variable is less than or equal to the
...
$y=3$ is obtained by the summation of the probabilities at $y=0$, $y=1$,
$y=2$, and $y=3$:

$$\Prob(Y \leq 3) = 0.0009765625 + 0.009765625 + 0.0439453125 + 0.1171875 = 0.171875$$
The numbers in the sum are the first 4 values from the output of the
function “`dbinom(y,10,0.5)`", which computes the probabilities of the
...
$$\Expec(Y) = \sum_y \big(x \times \Prob(Y = x)\big)\;,\quad \Var(Y) = \sum_y\big( (x-\Expec(Y))^2 \times \Prob(y)\big)\;.$$
However, in the specific case of the Binomial random variable, in which
the probability $\Prob(Y = y)$ obeys the specific mathematical formula
...
```{r}
Y.val <- 0:10
P.val <- dbinom(Y.val,10,0.5)
EY <- sum(Y.val*P.val)
EY
sum((Y.val-EY)^2*P.val)
```
...
products. The object “`Y.val`" contains all the values of the random
variable and the object “`P.val`" contains the probabilities of these
values. Hence, the expression “`Y.val*P.val`" produces the product of
...
expectation, which is saved in an object that is called “`EY`".
...
of all products. The expression “`(Y.val-EY)^ 2`" produces the sequence
...
“`(Y.val-EY)^2*P.val`") gives the variance.
...
plot(0:10, dbinom(0:10, 10,.5), type="h", ylab ="Probability", xlab="y", lwd=c(3), ylim=c(0,.4))
...
from the population. Let $Y$ be the count of supporters in the sample. A
natural model for the distribution of $Y$ is the
...
number of successes $Y$ has, approximately, the Poisson distribution.
...
plot(0:10, dpois(0:10, 2), type="h", ylab ="Probability", xlab="y", lwd=3)
```
...
expression “$Y \sim \mathrm{Poisson}(\lambda)$" states that the random
variable $Y$ has a Poisson distribution[^5_2] with expectation
$\Expec(Y) = \lambda$. The function “`dpois`" computes the probability,
...
cumulative probability at $y=7$ (the 8th value in the output of
“`ppois(y,2)`") is approximately 0.999, out of the total cumulative
...
plot(0:10, dpois(0:10,1), type="h", ylab ="Probability", xlab="y", lwd=c(3),ylim=c(0, .6))
...
to $\lambda$: $$\Expec(Y) = \Var(Y) = \lambda\;.$$
...
formula $\Expec(Y) = \sum_y \big(y \times \Prob(y)\big)$, which involves
...
$$\Expec(Y) = \int \big(y \times f(y)\big)dy\;,$$
where $f(y)$ is the density of $Y$ at the value $y$. Therefore, in the
...
$\Var(Y) = \sum_y\big( (y-\Expec(Y))^2 \times \Prob(y)\big)$ for the
variance is replaced by:

$$\Var(Y) =\int\big((y-\Expec(Y))^2 \times f(y) \big) dy\;.$$
...
For example, consider a random variable $Y$ with the Uniform
distribution over the interval $[3,7]$, denoted by
“$Y \sim \mathrm{Uniform}(3,7)$". The density function at given values
...
den <- dunif(y,3,7)
plot(y,den, type="l", ylab="Density",xlab="y", lwd=2)
...
uniform distribution. The probability $\Prob(Y \leq 4.73)$, for
$Y \sim \mathrm{Uniform}(3,7)$, is given by:
...
$y = 4.73$ in Figure \@ref(fig:RVar6). This area of the marked
...
“`y`". When we apply the function “`plot`" to the two sequences we get a
scatter plot of the 1,000 points.

A scatter plot is a plot of points. Each point in the scatter plot is
identified by its horizontal location on the plot (x-axis) and by
its vertical location on the plot (y-axis). The horizontal value
...
argument. For example, the first value in the sequence “`y`" is 0. The
...
histogram. Hence, if $Y \sim \mathrm{Uniform}(a,b)$ then:

$$\Expec(Y) = \frac{a+b}{2}\;.$$ For the $Y \sim \mathrm{Uniform}(3,7)$
distribution the expectation is $\Expec(Y)= (3+7)/2 = 5$. Observe that 5
is the center of the Uniform density in Plot \@ref(fig:RVar6).

It can be shown that the variance of the $\mathrm{Uniform}(a,b)$ is
equal to 

$$\Var(Y) = \frac{(b-a)^2}{12}\;,$$ with the standard deviation
being the square root of this value. Specifically, for
$Y \sim \mathrm{Uniform}(3,7)$ we get that
$\Var(Y) = (7-3)^2/12 = 1.333333$. The standard deviation is equal to
...
distribution via “$Y \sim \mathrm{Exponential}(\lambda)$", where
...
$Y \sim \mathrm{Exponential}(0.5)$. The density of the distribution in
...
y.c = seq(0,10,.1)
plot(c(0,y.c),c(0,dexp(y.c, .5)), type="l", ylab="Density",xlab="y")
y.c = seq(2,6,.1)
polygon(c(2,y.c,6),c(0,dexp(y.c, .5),0), density=10)
text(5.2,.22,"P(2 < y < 6)")
library(shape)
Arrows(4,.04,4.8, .18)
...
$Y \sim \mathrm{Exponential}(0.5)$. Say one is interested in the
computation of the probability $\Prob(2 < Y \leq 6)$ that the random
...
between the probability $\Prob(Y \leq 6)$, the area to the left of 6,
and the probability $\Prob(Y \leq 2)$, the area to the left of 2:
...
The expectation of $Y$, when $Y \sim \mathrm{Exponential}(\lambda)$, is
given by the equation: 

$$\Expec(Y) = 1/\lambda\;,$$ and the variance is
given by: 

$$\Var(Y) =1/\lambda^2\;.$$ The standard deviation is the
...
plot(c(0,x.coord), c(0,dexp(x.coord,1)), ylab ="Probability", lty=1, xlab="y",ylim=c(0, 2), type="l")
...
The fact that a random variable $Y$ has this distribution is marked by
“$Y \sim \mbox{Negative-Binomial}(r,p)$", where $r$ and $p$ are
parameters that specify the distribution.

Consider 3 random variables from the Negative-Binomial distribution:

-   $Y_1 \sim \mbox{Negative-Binomial}(2,0.5)$

-   $Y_2 \sim \mbox{Negative-Binomial}(4,0.5)$

-   $Y_3 \sim \mbox{Negative-Binomial}(8,0.8)$
...
1.  Produce bar plots of the distributions of the random variables
    $Y_1$, $Y_2$, $Y_3$ in the range of integers between 0 and 15 and
    ...
    function “`dnbinom(y,r,p)`", where “`y`" is a sequence of integers
...
2.  Below is a list of pairs that includes an expectation and a
    ...
    variables $Y_1$, $Y_2$, and $Y_3$:

    1.  $\Expec(Y) = 4$, $\Var(Y) = 8$.

    2.  $\Expec(Y) = 2$, $\Var(Y) =  4$.

    3.  $\Expec(Y) = 2$, $\Var(Y) =  2.5$.
...
$$\begin{aligned}
        \Expec(Y) &= \int \big(y \times f(y)\big)dy \\
        \Var(Y) &= \int\big((y-\Expec(Y))^2 \times f(y) \big) dy \end{aligned}$$
...
:   $\Expec(Y) = \lambda\;, \quad \Var(Y) = \lambda$

Uniform:

:   $\Expec(Y) = (a+b)/2\;, \quad \Var(Y)= (b-a)^2/12$

Exponential:

:   $\Expec(Y) = 1/\lambda\;, \quad \Var(Y)= 1/\lambda^2$

[^5_1]: If $Y\sim \mathrm{Binomial}(n,p)$ then
    $\Prob(Y = y) = {n \choose y} p^y (1-p)^{n-y}$, for
    $y = 0, 1, \ldots, n$.

[^5_2]: If $Y \sim \mathrm{Poisson}(\lambda)$ then
    $\Prob(Y=y) = e^{-\lambda}\lambda^y/y!$, for $y=0,1,2,\ldots$.
...
[^5_4]: If $Y \sim \mathrm{Uniform}(a,b)$ then the density is
    $f(y) = 1/(b-a)$, for $a \leq y \leq b$, and it is equal to 0 for
    other values of $y$.

[^5_5]: If $Y \sim \mathrm{Exponential}(\lambda)$ then the density 
    is $f(y) =\lambda e^{-\lambda y}$, for $0 \leq y$, and it is equal 
    to 0 for $y < 0$.

[^5_6]: It can be shown, or else found on the web, that if
    $Y\sim \mbox{Negative-Binomial}(r,p)$ then $\Expec(Y) = r(1-p)/p$
    and $\Var(Y) = r(1-p)/p^2$.
