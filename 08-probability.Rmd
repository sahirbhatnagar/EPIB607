...
```{r frogBoxPlot2, echo=FALSE, fig.cap='A boxplot and dot plot of `clutch.volume`. The horizontal dashes indicate the bottom 50% of the data and the open circles represent the top 50%.'}
library(oibiostat)
library(openintro)
data(COL)
data(frog)
...
```{r Prob1, fig.cap='Bar Plot of Height', echo=TRUE, message=FALSE,warning=FALSE, out.width = '60%', fig.align = "center"}
library(ggplot2)
library(cowplot)
...
y <- c(0,1,2,3)
p <- c(.5,.25,.15,.1)
tab1 = data.frame(y=y,p=p,py=p*y)
knitr::kable(
  tab1, 
  booktabs = TRUE,
...
ym = y-sum(p*y)
yy = data.frame(y,p,ym,ym2=ym^2,ym2p=p*ym^2)
knitr::kable(
  yy, 
  booktabs = TRUE,
  col.names= c("$y$", "$P(Y=y)$", "$y - E[Y]$", "$(y - E[Y])^2$", "$P(Y=y)*(y - E[Y])^2$"),
  caption = 'The Variance of $Y$'
)
```
