...
x.coord = seq(-10,10,.1)
plot(c(0,x.coord), c(0,dnorm(x.coord,0,1)), ylab ="Density", lty=1, xlab="y",ylim=c(0, .8), type="l", lwd = 3)
lines(c(0,x.coord)-.03, c(0,dnorm(x.coord,2,3)), col=2, lwd = 3)
lines(c(0,x.coord)+.03, c(0,dnorm(x.coord,-3,sqrt(.25))), col=3, lwd = 3)
legend(4, .8, c("N(0,1)", "N(2,3)", "N(-3,0.50)"), col = c(1, 2, 3), lty = c(1, 1, 1), lwd = 3)
```
...
```{r Normal4, fig.cap='Central 95$\\%$ of the Standard Normal Distribution',echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center"}
...
```{r pqfunction,eval=T, echo=F, fig.align="center", fig.cap = 'p and q functions visual representation.', warning=FALSE, message=F}
