...
[^random-variables-2]
   + Let $X$ be the possible earnings $\mathbb{E}(X) = \sum_{i=1}^6 x_i\cdot\mathbb{P}(X=x_i) = 309.741$
   + (a) By definition: $Var(X)=\sum_{i=1}^6 \big(x_i-\mathbb{E}(X)\big)^2\cdot\mathbb{P}(X=x_i) = 3806716$, and thus the SD is $sd(X)=\sqrt{Var(X)} = 1951.081$
   \hspace{0.5cm} (b) By shortcut: $Var(X)=\mathbb{E}(X^2) - \mathbb{E}(X)^2=\sum_{i=1}^6x_i^2\cdot\mathbb{P}(X=x_i) - \mathbb{E}(X)^2=3806716$, the same as (a). 
   + The risk $=\frac{sd(X)}{\mathbb{E}(X)}\approx 629.91\%$ which indicating a very high risk.
   + In statistics, it is called the coefficient of variation (CV).
...
[^random-variables-3] 
  + If the ending numbers of the unrounded data were uniformly distributed from 0 to 9, denote the error of rounding by $X$, then $X\sim Unif(\{-4,-3,cdots,4,5\})$ (i.e. each error value with probability 1/10).
  $\mathbb{E}(X)=\sum_{i=1}^{10} x_i\cdot\mathbb{P}(X=x_i) = 0.5$, the rounding on average makes an observation larger by 0.2. 
  + The average absolute error $\mathbb{E}(|X|)=\sum_{i=1}^{10} |x_i|\cdot\mathbb{P}(X=x_i) = 2.5$ 
  + RMSE = $\sqrt{\sum_{i=1}^{10}(x_i)^2/10}\approx 2.91$
...
[^random-variables-4]
    + Let $X_i$ be 1 if the student knows the answer to $i^{th}$ question, and 0 otherwise. Let $Y_i$ be the mark he get for question $i$. For a question this student don't know the answer to, he randomly guesses, and $\mathbb{P}(Y_i=1|X_i=0) = \frac{1}{k}$. For a question this student know the answer to, he answer it correctly with $\mathbb{P}(Y_i=1|X_i=1)=1$. Hence, the expected proportion of correctly answered question is:
    $\mathbb{E}(p)=\mathbb{P}(Y_i=1)=\mathbb{P}(Y_i=1|X_i=0)\cdot\mathbb{P}(X_i=0) + \mathbb{P}(Y_i=1|X_i=1)\cdot\mathbb{P}(X_i=1) = \frac{1}{k}\cdot (1-\pi) + 1*\pi>\pi$
  Hence it will always be a biased estimates (over-estimated) of $\pi$.
  If you are drawing a tree diagram, split the first node by whether this student knows the answer, then split the second level nodes by whether he answer it correctly.
    + Now, $Y_i=1$ if the answer is correct and $Y_i=b$ ($b$ is negative in this case) if the answer is wrong. The expected mark this student gets is:
    $\mathbb{E}(Y_i)=1\cdot\mathbb{P}(Y_i=1)+b\cdot\mathbb{P}(Y_i=b)$
   We have the probability $\mathbb{P}(Y_i=1)=\frac{1-\pi}{k}+\pi$ from part (1), and since they student can only give correct or wrong answer, $\mathbb{P}(Y_i=b)=\mathbb{P}(\text{wrong answer})=1-\frac{1-\pi+k\pi}{k}=\frac{(k-1)(1-\pi)}{k}$
   Now plug in to $\mathbb{E}(Y_i)$ and set it equals to $\pi$, we have: $b=\frac{1}{1-k}$ (note that for multiple choices, $k>1$, so $b$ is negative.)
...
[^random-variables-5] Let $X$ be the number of eggs purchase $\mathbb{E}(X)=6*\frac{1}{2} + 12*\frac{1}{2} = 9$. The standard deviation is $sd(X)=\sqrt{Var(X)} = \sqrt{\underbrace{\mathbb{E}(X^2)}_{=6^2*\frac{1}{2} + 12^2*\frac{1}{2}}-\mathbb{E}(X)^2}=3$ 

Both 6 eggs and 12 eggs purchase are considered to be within 1 SD from the mean ($9\pm 3$). Hence, 0\% of the purchases are more than 1 SD from the mean and 100\% are within 1 SD.

6. Half the people in a population have 2 organs of a certain type and half have none. What is the standard deviation of the number of organs a randomly selected person has? 
[^random-variables-6] Let $X$ be the number of organs for one person. The chance of a randomly selected person having two organs is 1/2 and the chance of him having 0 organs is 1/2.
$$sd(X)=\sqrt{Var(X)}=\sqrt{\mathbb{E}(X^2)-\mathbb{E}(X)^2} =1$$
...
% do not understand what the question is asking about
...
[^random-variables-8] Denote the CF gene to be 'A' and the normal gene to be 'a'. Then the offspring can have 4 genotypes, each of the same probability (1/4): 
\begin{itemize}
\item 'AA' (getting two copies of CF, each from one parent), 
\item 'Aa' (getting one copy of CF from father, and one copy of normal from mother)
\item 'aA' (getting one copy of normal from father, and one copy of CF from mother)
\item 'aa' (getting two copies normal, each from one parent)
\end{itemize}

Hence 1/4 of their offspring carry 0 copy, 1/2 carry 1 copy and 1/4 carry 2 copies.
...
[^random-variables-9]  Let $X$ be the shipping time (in days). Tuesday orders can arrive at next Mon ($X=6$), Wed ($X=8$) and Fri($X=10$) with equal probability (1/3). Similarly, Thursday orders can arrive at next Mon ($X=4$), Wed($X=6$) and Fri($X=8$) with equal probability. The probability tree can be drawn:

```{tikz, ThreeStateDiagram, fig.cap="Probabiity Tree of the Shipping Time", fig.align="center"}
...
[^random-variables-10]
    + If the two readings are at 12.5 and 22.5 in Celsius, the true Fahrenheit readings are 54.5 and 72.5. Due to the rounding, there are 4 possibilities in the displayed Fahrenheit: (54,72), (54,73), (55,72), and (55,73). The estimates of the scaling factor $\beta$ are 1.8, 1.9, 1.7, and 1.8, respectively, each with probability 1/4. $\mathbb{E}(\hat{\beta})=1.8$
    + $Var[\hat{\beta}] = \frac{1}{4}\Big((1.8-1.8)^2+(1.9-1.8)^2+(1.7-1.8)^2+(1.8-1.8)^2\Big)=0.005$, only 1/4 of the variance in previous example. The SE is approximately 0.07, half of the SE in previous example.
    + ? With more than two readings, you can estimate $\beta$ from each pair of readings and take the average. There are $2^4=16$ combinations of the 4 readings in Fahrenheit. In each combination, you need to calculate the estimated $\beta$ 6 times from 6 pairs (\#1,\#2), (\#1,\#3), (\#1, \#4), (\#2, \#3), (\#2, \#4) and (\#3, \#4).  Theoretically, the more readings you have, the smaller variance of the estimates is.
    + With a larger variance in the measurement error, the estimated slope will have a larger variance as $Var[\hat{\beta}] \propto Var(e) $ where $e$ is the measurement error.
    
    
    
11. Random Variable: Are the following DISCRETE or CONTINUOUS?

* _How long_ you have to wait for bus / elevator / surgery/ download to complete \textbf{CONTINUOUS}
* the _blood group_ of n = 1 randomly selected person \textbf{DISCRETE}
* _how many tries_ before pass a course \textbf{DISCRETE}
* _how many_ of n = 20 randomly selected persons will return questionnaire in pilot study \textbf{DISCRETE}
* _length of song_ on a CD \textbf{CONTINUOUS}
* _mean cholesterol level_ in sample of n = 30 randomly selected persons \textbf{CONTINUOUS}
* _how hot_ it is going to be today \textbf{CONTINUOUS}
* _how much_ snow we will get next winter \textbf{CONTINUOUS}
* _time_ someone called (on answering machine) \textbf{CONTINUOUS}
* _value of test-statistic_ if 2 populations sampled from have the same mean \textbf{CONTINUOUS}
* _how much_ ice McDonalds puts in soft drink \textbf{CONTINUOUS (in grams)}
* _how many_ calories in hamburger \textbf{CONTINUOUS}
* _how many_ numbers you get correct in 6/49?  \textbf{DISCRETE}
* _where_ roulette wheel stops \textbf{DISCRETE}
* _how many_ "wrong number" calls received  \textbf{DISCRETE}
* _how many_ keys you have to try before get the right one  \textbf{DISCRETE}
* _how much_ water consumed by 100 homes \textbf{CONTINUOUS}

Note: we only consider the  most commonly used measurement in each example.
