...
```{r 5cars,  echo=FALSE, fig.cap = "A sample of five cars from Awesome Auto.", out.width = "75%", fig.alt = "Photographs of 5 different automobiles.  The cars are different color and different makes and models.  On top of the image of each car is its price; the five prices range from 9600 dollars to 27000 dollars."}
include_graphics(here::here("inst/figures/5cars.png"))
```
...
```{r bootpop1mean,  echo=FALSE, fig.cap = "As seen previously, the idea behind bootstrapping is to consider the sample at hand as an estimate of the population.  Sampling from the sample (of 5 cars) is identical to sampling from an infinite population which is made up of only the cars in the original sample.", fig.alt = "The sample of 5 cars is drawn from a large population with other values (i.e., other car prices) unknown. The sample of five cars is replicated infinitely many times to create a proxy population where the car prices are given by the original dataset in the same relative distribution as measured in the sample."}
include_graphics(here::here("inst/figures","bootpop1mean.png"))
```
...
```{r bootsamps1mean, echo=FALSE,  fig.cap = "To estimate the natural variability in the sample mean, different bootstrap samples are taken from the original sample.  Notice that each bootstrap resample is different from each other as well as from the original sample", fig.alt = "The sample is shown being taken from the large unknown population.  The bootstrap resamples, however, are taken directly from the original sample (sampling with replacement) as if the resamples had been taken from an infinitely large proxy population.  Three bootstrap resamples of 5 cars each are shown, each resample is slightly different due to the process of resampling with replacement."}
include_graphics(here::here("inst/figures","bootsamps1mean.png"))
```
...
```{r bootmeans1mean,  echo=FALSE, fig.cap = "Because each of the bootstrap resamples respresents a different set of cars, the mean of the each bootstrap resample will be a different value.  Each of the bootstrapped means is calculated, and a histogram of the values describes the inherent natural variability of the sample mean which is due to the sampling process.", out.width="100%", fig.alt = "The sample is shown being taken from the large unknown population.  Three bootstrap resamples of 5 cars each are shown; the three resamples have an average car price of 11780 dollars, 19020 dollars, and 20260 dollars, respectively. A histogram representing many bootstrap resamples indicates that the bootstrap averages vary from roughly 10000 dollars to 25000 dollars."}
include_graphics(here::here("inst/figures","bootmeans1mean.png"))
```
...
```{r carsbsmean,  echo=FALSE, fig.cap = "The original Awesome Auto data is bootstrapped 1,000 times. The histogram provides a sense for the variability of the average car price from sample to sample.", out.width = "100%"}
set.seed(47)
bscars <- aa_cars %>%
...
```{r carsbssd,  echo=FALSE, fig.cap = "The original Awesome Auto data is bootstrapped 1,000 times. The histogram provides a sense for the variability of the standard deviation of the car prices from sample to sample.", out.width = "100%"}
bscars_sd <- bscars %>%
  group_by(replicate) %>% 
...
```{r outliersandsscondition,  echo=FALSE, fig.cap = "Histograms of samples from two different populations."}
set.seed(3456)
...
```{r tDistCompareToNormalDist,  echo=FALSE, fig.cap = "Comparison of a $t$-distribution and a normal distribution.", fig.asp = 0.5, out.width = "60%"}
df <- tibble(
...
```{r tDistConvergeToNormalDist,  echo=FALSE, fig.cap="The larger the degrees of freedom, the more closely the $t$-distribution resembles the standard normal distribution."}
...
```{r tDistDF18LeftTail2Point10, echo=FALSE,  fig.cap = "The $t$-distribution with 18 degrees of freedom. The area below -2.10 has been shaded.", fig.asp = 0.5, out.width = "60%"}
normTail(L = -2.10,
         df = 10,
         xlim = c(-4, 4),
...
```{r tDistDF20RightTail1Point65,  echo=FALSE, fig.cap = "Top: The $t$-distribution with 20 degrees of freedom, with the area above 1.65 shaded.", out.width="50%"}
normTail(U = 1.65,
  df = 12,
  xlim = c(-4, 4),
  col = IMSCOL["blue", "full"])
...
```{r tDistDF23UnitsFromMean,  echo=FALSE, fig.cap = "The $t$-distribution with 2 degrees of freedom, with the area further than 3 units from 0 shaded.", out.width="50%"}
normTail(L = -3,
  U = 3,
  df = 2.3,
  xlim = c(-4.5, 4.5),
...
```{r rissosDolphin,  echo=FALSE, fig.cap = "A Risso's dolphin. Photo by Mike Baird, www.bairdphotos.com", out.width="75%", fig.alt = "Photograph of a Risso's dolphin."}
include_graphics(here::here("inst/figures","rissosDolphin.jpg"))
```
...
```{r summaryStatsOfHgInMuscleOfRissosDolphins,  echo=FALSE}
