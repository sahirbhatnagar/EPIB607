...
kbl(cbind(famuss.race.row.prop,Sum = apply(famuss.race.row.prop, 1, sum)), 
...
```{r famussColPropTable, echo=FALSE}
data(famuss); 
famuss.race = table(famuss$race, famuss$actn3.r577x); 
famuss.race.col.prop = prop.table(famuss.race, 2); 
kbl(rbind(famuss.race.col.prop, Sum = apply(famuss.race.col.prop, 2, sum)), 
       digits = 3, 
       caption = "A contingency table with column proportions for the `race` and `actn3.r577x` variables.") %>% 
  kable_styling()
```
...
A **segmented bar plot** is a way of visualizing the information from a contingency table. Figure \@ref(fig:famussSegBarPlotA) graphically displays the data from Table \@ref(tab:famussContingencyTable); each bar represents a level of `actn3.r577x` and is divided by the levels of `race`. Figure \@ref(fig:famussSegBarStaA) uses the row proportions to create a standardized segmented bar plot. 


```{r famussSegBarA, fig.cap='Segmented bar plot for individuals by genotype, with bars divided by race', echo=FALSE}
# library(openintro)
#data(famuss)
data(COL)
...
```{r famussSegBarStaA, fig.cap='Standardized version of Figure \\@ref(fig:famussSegBarA)', echo=FALSE}
#sta seg bar
...
Alternatively, the data can be organized as shown in Figure \@ref(fig:famussSegBarPlotB), with each bar representing a level of `race`. The standardized plot is particularly useful in this case, presenting the distribution of genotypes within each race more clearly than in Figure \@ref(fig:famussSegBarB).


```{r famussSegBarB, fig.cap='Segmented bar plot for individuals by race, with bars divided by genotype', echo=FALSE}
## races as columns
...
```{r famussSegBarStaB, fig.cap='Standardized version of Figure \\@ref(fig:famussSegBarB)', echo=FALSE}
#sta seg bar
