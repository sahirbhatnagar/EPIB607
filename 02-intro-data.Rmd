...
In a **relative frequency table** like Figure \@ref(fig:famussRelFrequencyTable}, the proportions per each category are shown instead of the counts.
...
```{r frogClutchVolBodySize, echo=FALSE, fig.cap='A scatterplot showing `clutch.volume` (vertical axis) vs. `body.size` (horizontal axis). '}
data(frog)
data(COL)
frog.altitude <- frog

par(  mar = c(3, 4.5, 0.5, 1.2),
      mgp = c(2.8, 0.5, 0))
...
```{r nhanesHeightWeight, echo=FALSE, fig.cap='A scatterplot showing height versus weight from the 500 individuals in the sample from `NHANES`. One participant 163.9 cm tall (about 5 ft, 4 in) and weighing 144.6 kg (about 319 lb) is highlighted.'}
data(COL)
...
```{r nhanesHeightBMI, fig.cap='A scatterplot showing height versus BMI from the 500 individuals in the sample from `NHANES`. The same individual highlighted in Figure \\@ref(fig:nhanesHeightWeight) is marked here, with BMI 53.83.', echo=FALSE}
...
```{r incomeLifeExpectancy, echo=FALSE, fig.cap='A scatterplot of life expectancy (years) versus annual per capita income (US dollars) in the `wdi.2011` dataset from the oibiostat package.'}
...
```{r posNegCorPlots, fig.cap='Scatterplots and their correlation coefficients. The first row shows positive associations and the second row shows negative associations. From left to right, strength of the linear association between $x$ and $y$ increases.', echo=FALSE}
...
CorrelationPlot(possum$total_l, possum$head_l)
...
```{r corCalcSimple, echo=FALSE, fig.cap='A scatterplot showing three points: (1, 5), (2, 4), and (3, 0)'}
...
```{r incomeLifeExpectancyLog, fig.cap='A scatterplot showing `log(income)` (horizontal axis) vs.  `log(life.expectancy)` (vertical axis).', echo=FALSE}
data("wdi.2011")
data(COL)
...
```{r famussContingencyTable, echo=FALSE}
data(famuss); 
genotype.by.race.table = addmargins(table(famuss$race, famuss$actn3.r577x)); 

kbl(genotype.by.race.table, 
    caption = "A contingency table for race and actn3.r577x.") %>% 
  kable_styling()
```
...
```{r famussRowPropTable, echo=FALSE}
data(famuss); 
famuss.race = table(famuss$race, famuss$actn3.r577x); 
famuss.race.row.prop = prop.table(famuss.race, 1); 

 
kbl(famuss.race.row.prop, 
    caption = "A contingency table with row proportions for the race and actn3.r577x variables.") %>% 
  kable_styling()
```



Table \@ref(tab:famussColPropTable) shows the column proportions for Table \@ref(fig:famussContingencyTable); these proportions indicate the distribution of races within each genotype category. For example, the value of 0.092 indicates that of the CC individuals in the study, 9.2% are African American. 
...
\begin{exercisewrap}
\begin{nexercise}
As shown in Figure \@ref(fig:famussColPropTable}, 72.3\% of CC individuals in the study are Caucasian. Do these data suggest that in the general population, people of CC genotype are highly likely to be Caucasian?\footnotemark{}
\end{nexercise}
\end{exercisewrap}
...
A \termsub{segmented bar plot}{bar plot!segmented bar plot} is a way of visualizing the information from a contingency table. Figure \@ref(fig:famussSegBarPlotA} graphically displays the data from Figure \@ref(fig:famussContingencyTable}; each bar represents a level of \var{actn3.r577x} and is divided by the levels of \var{race}. Figure \@ref(fig:famussSegBarStaA} uses the row proportions to create a standardized segmented bar plot. 
...
Alternatively, the data can be organized as shown in Figure \@ref(fig:famussSegBarPlotB}, with each bar representing a level of \var{race}. The standardized plot is particularly useful in this case, presenting the distribution of genotypes within each race more clearly than in Figure \@ref(fig:famussSegBarB}.
...
The results from medical studies are often presented in **two-by-two tables} (2 $\times$ 2 tables), contingency tables for categorical variables that have two levels. One of the variables defines two groups of participants, while the other represents the two possible outcomes. Figure \@ref(fig:TwoByTwoTable} shows a hypothetical two-by-two table of outcome by group.
...
In the LEAP study, participants are divided into two groups based on treatment (peanut avoidance versus peanut consumption), while the outcome variable records whether an individual passed or failed the oral food challenge (OFC). The results of the LEAP study as shown in Figure \@ref(fig:leapStudyResults} are in the form of a 2 $\times$ 2 table; the table is reproduced below as Figure \@ref(fig:leapStudyResultsRR}.

A statistic called the **relative risk} (RR) can be used to summarize the data in a 2 $\times$ 2 table; the relative risk is a measure of the risk of a certain event occurring in one group relative to the risk of the event occurring in another group.\footnote{Chapter~\ref{inferenceForCategoricalData} discusses another numerical summary for 2 $\times$ 2 tables, the **odds ratio}.}  
...
\begin{onebox}{Relative risk}%	
		The relative risk of Outcome A in the hypothetical two-by-two table (Figure \@ref(fig:TwoByTwoTable}) can be calculated using either Group 1 or Group 2 as the reference group:
...
\begin{exercisewrap}
\begin{nexercise}
Using Figure \@ref(fig:frogClutchVolAlt}, assess how maternal investment varies with altitude.\footnotemark{}
\end{nexercise}
\end{exercisewrap}
